...
#define D e_remember_edd
   ...
   E_CONFIG_VAL(D, T, prop.xkb, UCHAR);
...
#define D _e_config_edd
   ...
   E_CONFIG_VAL(D, T, systray_on_demand, INT); /**/
...
#define CONFIG_VERSION_UPDATE_INFO(VERSION) \
  INF("Performing config upgrade for %d.%d", E_CONFIG_FILE_EPOCH, VERSION);
   ...
   if (e_config->config_version < E_CONFIG_FILE_VERSION)
     ...
     E_CONFIG_LIMIT(e_config->systray_on_demand, 0, 1);
...
e_config_profile_save(void)
{
   ...
   if (nosave == -1)
     {
        if ((s = getenv("M_CONF_PROFILE_NOSAVE")) && atoi(s)) nosave = 1;
        else nosave = 0;
     }
