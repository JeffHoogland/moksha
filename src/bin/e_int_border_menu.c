...
static Eina_Bool
e_border_util_borderless(const E_Border *bd)
{
   if (!bd) return EINA_FALSE;
   return (bd->borderless || bd->client.mwm.borderless || (!bd->client.border.name) || (!strcmp(bd->client.border.name, "borderless")));
}
...
e_int_border_menu_create(E_Border *bd)
{
   ...
   borderless = e_border_util_borderless(bd);
   ...
   if ((!bd->sticky) && ((eina_list_count(bd->zone->container->zones) > 1) ||
      ((bd->zone->desk_x_count > 1) || (bd->zone->desk_y_count > 1))))
     {
        mi = e_menu_item_new(m);
   ...
   if ((!bd->fullscreen) && (!bd->lock_border) && (!bd->shading) &&
       (!bd->shaded))
     {
        mi = e_menu_item_new(m);
...
_e_border_menu_cb_border_pre(void *data, E_Menu *m __UNUSED__, E_Menu_Item *mi)
{
   ...
   submi = e_menu_item_new(subm);
   e_menu_item_label_set(submi, _("Use Moksha Default Icon Preference"));
   e_menu_item_radio_set(submi, 1);
   e_menu_item_radio_group_set(submi, 2);
