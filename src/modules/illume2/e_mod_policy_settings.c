...
_il_config_policy_settings_ui(E_Config_Dialog *cfd, Evas *evas, E_Config_Dialog_Data *cfdata) 
{
   ...
   otop = e_widget_radio_add(evas, _("Top/Bottom"), 0, rg);
   e_widget_framelist_object_append(of, otop);
   evas_object_smart_callback_add(otop, "changed", 
                                  _il_config_policy_settings_changed, NULL);
   oleft = e_widget_radio_add(evas, _("Left/Right"), 1, rg);
   e_widget_framelist_object_append(of, oleft);
   evas_object_smart_callback_add(oleft, "changed", 
                                  _il_config_policy_settings_changed, NULL);
   e_widget_list_object_append(list, of, 1, 0, 0.0);
